...
def run(encoder, predictor, split_edge, data, optimizer, adj_list_dict, args, wandb):
    ...
    if args.train:
        for epoch in range(1, 1 + args.epochs):
            ...
            if args.aug and epoch >= args.warm_up and epoch%args.update_interval == 0:
                # print(222)
                data.train_adj_aug = update_adj(encoder, predictor, data, epoch, args)
...
if __name__ == '__main__':
    ...
    elif args.dataset in ['Cora', 'Citeseer', 'Pubmed', 'Reptile', 'Vole']:
    ...
    for args.run in pbar:
        ...
        """build dataset"""
        data, split_edge, adj_list_dict = load_data(args)

        # torch.save(split_edge, './data/' + args.dataset + '/split_edge_' + str(args.run))
        # torch.save(data, './data/' + args.dataset + '/data_' + str(args.run))

        
        
        # print('# of nodes:', data.n_nodes)
        # print('# of edges:', data.n_edges)
        # print('# of training edges:', split_edge['train']['edge'].shape[0])
        # print('# of validation edges:', split_edge['valid']['edge'].shape[0])
        # print('# of testing edges:', split_edge['test']['edge'].shape[0])
        ...
        if args.wandb and args.train:
            wand_setting = wandb.init(
                                project="TC-NIPS-{}".format(args.dataset),
                                notes="",
        ...
        if args.dataset in ['Reptile', 'Vole']:
            emb = torch.nn.Embedding(data.x.size(0), args.n_hidden).to(args.device)
            data.x = emb.weight
        ...
        if args.encoder == 'GCN':
            ...
            encoder = SAGE(data.x.shape[1], args.n_hidden, args.n_hidden, args.n_layers, args.en_dp, normalize = args.sage_norm).to(args.device)
        ...
        if args.encoder in ['GCN', 'SAGE']:
            if args.dataset in ['Reptile', 'Vole']:
                optimizer = torch.optim.Adam([{'params': encoder.parameters(), "lr": args.encoder_lr},
                                              ...
                                              {'params': emb.parameters(), 'lr': args.encoder_lr}])
            else:
                optimizer = torch.optim.Adam([{'params': encoder.parameters(), "lr": args.encoder_lr},
                                              {'params': predictor.parameters(), 'lr': args.predictor_lr}])

        ress_final = run(encoder, predictor, split_edge, data, optimizer, adj_list_dict, args, wandb)
